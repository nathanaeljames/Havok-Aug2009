inline void reset_mixalnumsymb32_L5(char candidate[], unsigned int position[])
{
...
void hba_L5_mixalnumsymb32(char candidate[], char permutations[], uint8_t *mscachehash, char u_username[], int sUlen)
{
	...
	reset_mixalnumsymb32_L5(candidate,position);//reset positions from password
						...
						for(;permutated<=2147000000 && position[2]<charcount;position[2]++)
						{
							...
							for(;permutated<=2147000000 && position[1]<charcount;position[1]++)
							{
								...
								for(;permutated<=2147000000 && position[0]<charcount;position[0]++)
								{
									...
									fast_MD4((unsigned char*)uni_pwd,10,K);
