...
void hba_L10_mixalnumsymb32(char candidate[], char permutations[], uint8_t *md5hash)
{
							...
							for(;permutated<=2147000000 && position[2]<charcount;position[2]++)
							{
								...
								for(;permutated<=2147000000 && position[1]<charcount;position[1]++)
								{
									...
									for(;permutated<=2147000000 && position[0]<charcount;position[0]++)
									{
										...
										//cout << candidate << "\n";
										//MD4_Init(&ctx);
										//MD4_Update(&ctx,candidate,10);
										//MD4_Final(K,&ctx);
										fast_MD5((unsigned char*)candidate,10,K);
										//dump(K);
										if(!memcmp(md5hash,K,16))
										{
											cout << "plaintext of ";
											dump(K);
											cout << " is " << candidate << "\n";
